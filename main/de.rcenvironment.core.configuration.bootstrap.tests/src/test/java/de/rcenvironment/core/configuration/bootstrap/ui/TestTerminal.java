...
/**
 * This is a terminal that can be used to test Lanterna UIs. It allows to access the screen content.
 *
...
public class TestTerminal implements Terminal {
     ...
     */
    private LinkedList<char[][]> previousContent;

    private FlushListener listener;

    private Log log = LogFactory.getLog(TestTerminal.class);

    public TestTerminal(int columns, int rows) {

        this.size = new TerminalSize(columns, rows);
        this.content = new char[rows][columns];
        this.keyQueue = new LinkedList<Key>();
        this.previousContent = new LinkedList<char[][]>();
    }
    ...
    public synchronized void flush() {
        log.debug("flush");
        for (int i = 0; i < content.length; i++) {
            log.debug(new String(content[i]));
        }
