...
public class ComponentRegistryImpl implements ComponentRegistry {
    ...
    /**
     * Needed to remove components from {@link ComponentRegistry} which are
     * added/removed by OSGi dependency injection.
     */
    ...
    protected synchronized void activate(org.osgi.service.component.ComponentContext newContext) {
        ...
        localNode = platformService.getLocalDefaultLogicalNodeId();

        synchronized (unhandledCompControllers) {
            osgiComponentCtx = newContext;
            // TODO >6.0.0 rework; bridge code to map the new configuration
            // layout onto the old java bean
            ConfigurationSegment configurationSegment = configurationService.getConfigurationSegment("publishing");
            try {
                // simple migration path for now: mapping the whole of
                // /components onto the old object.
                // this will need to be changed to expand to more configuration
                // settings - misc_ro
        ...
        if (!CommandLineArguments.isDoNotStartComponentsRequested()) {
            ...
            ConcurrencyUtils.getAsyncTaskService().execute(r);
        } else {
            LOGGER.debug("Not triggering start of remaining component bundles as component loading is disabled");
        }

        // start publishing of components after short delay to avoid frequent
        // updates during initial
    ...
    /**
     * Bind method called by the OSGi framework, if a new Component Factory of
     * type de.rcenvironment.rce.component was registered. The component
     * registration stuff is done here.
     */
        ...
        if (compInstance != null) {
            try {
                ...
                if (componentReference != null) {
                    ...
                    if (componentRevision != null) {
                        ...
                        compFactoryServiceIdToCompInstIdMapping.put(
                                factoryReference.getProperty(Constants.SERVICE_ID).toString(),
                ...
                try {
                    ...
                    // OSGi complains if no unbind method was declared in
                    // ServiceComponent for a service reference. As we cannot be
                    // sure,
                    // each
                    // component developer is aware of that, a possible NPE is
                    // caught here
    ...
    public void addComponent(ComponentInstallation componentInstallation) {
        final boolean toPublish = isAllowedToBePublished(componentInstallation);
        synchronized (installationLock) {
        ...
        LOGGER.debug(StringUtils.format("Registered component: %s (published: %s)", componentInstallation.getComponentRevision()
            .getComponentInterface().getIdentifier(), toPublish));
    }
    ...
    private void publishComponents() {
        if (publishDelayPast) {
            componentDistributor.setLocalComponentInstallations(localInstallations.values(),
                    publishedInstallations.values());
        } else {
            componentDistributor.setLocalComponentInstallations(localInstallations.values(),
                    new ArrayList<ComponentInstallation>());
        }
    }
    ...
    private ComponentInstallation createComponentInstallation(ComponentRevisionImpl componentRevision) {
        ...
        componentInstallation.setNodeIdFromObject(localNode);
    ...
    private void setIdentifiers(ServiceReference<?> componentReference, ComponentInterfaceImpl componentInterface) {
        ...
        // use explicitly given id(s) if present, otherwise generate id the
        // "old" way.
    ...
    private ComponentRevisionImpl createComponentRevision(ServiceReference<?> componentReference) {
        ...
        // read and process all the service properties. actually, they are the
        // properties of the
        // component
