...
public class TypedDatumToOutputWriterTest {
    ...
    private static final NodeIdentifier OUTP_COMP_NODE_ID = NodeIdentifierFactory.fromNodeId("outp-comp-node-id");
    
    private static final String WF_CTRL_EXE_ID = "wf-ctrl-exe-id";
    
    private static final NodeIdentifier WF_CTRL_NODE_ID = NodeIdentifierFactory.fromNodeId("wf-ctrl-node-id");
    ...
    private void testWritingTypedDatumToOutput(Long dmId) {
        ...
        assertEquals(OUTP_COMP_NODE_ID, epCapture1.getValue().getOutputsNodeId());
        assertEquals(WF_CTRL_EXE_ID, epCapture1.getValue().getWorkflowExecutionIdentifier());
        assertEquals(WF_CTRL_NODE_ID, epCapture1.getValue().getWorkflowNodeId());
    ...
    public void testWritingTypedDatumToConnectedOutputConsideringTargetInput() {
        ...
        assertEquals(OUTP_COMP_NODE_ID, epCapture.getValue().getOutputsNodeId());
        assertEquals(WF_CTRL_EXE_ID, epCapture.getValue().getWorkflowExecutionIdentifier());
        assertEquals(WF_CTRL_NODE_ID, epCapture.getValue().getWorkflowNodeId());
    ...
    private ComponentExecutionContext createComponentExecutionContextMock() {
        ...
        EasyMock.expect(compExeCtxMock.getNodeId()).andStubReturn(OUTP_COMP_NODE_ID);
        EasyMock.expect(compExeCtxMock.getWorkflowExecutionIdentifier()).andStubReturn(WF_CTRL_EXE_ID);
        EasyMock.expect(compExeCtxMock.getWorkflowNodeId()).andStubReturn(WF_CTRL_NODE_ID);
