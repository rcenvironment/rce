 ...
 */
public class SqlReaderComponentHistoryDataItemSubtreeBuilder extends DefaultHistoryDataItemSubtreeBuilder {

    private static final Image COMPONENT_ICON;

    static {
        String iconPath = "platform:/plugin/de.rcenvironment.components.sql.common/resources/sqlReader_16.png";
        URL url = null;
        try {
            url = new URL(iconPath);
        } catch (MalformedURLException e) {
            LogFactory.getLog(SqlReaderComponentHistoryDataItemSubtreeBuilder.class).error("Component icon not found: " + iconPath);
        }
        if (url != null) {
    ...
    public String[] getSupportedHistoryDataItemIdentifier() {
        return new String[] { SqlComponentConstants.COMPONENT_ID_READER };
    }
