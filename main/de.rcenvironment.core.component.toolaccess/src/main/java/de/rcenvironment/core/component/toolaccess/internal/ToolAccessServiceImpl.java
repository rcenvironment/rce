 ...
 */
public class ToolAccessServiceImpl {
    ...
    private final HeadlessWorkflowExecutionService workflowExecutionService;
    ...
    public ToolAccessServiceImpl(HeadlessWorkflowExecutionService workflowExecutionService) {
        // TODO rework to a "real" service with dependency injection when necessary
    ...
    private boolean validateWorkflowFileAsTemplate(WorkflowDescription wd, TextOutputReceiver outputReceiver)
        throws WorkflowExecutionException {
        validateEquals(Integer.valueOf(3), wd.getWorkflowVersion(), "Invalid workflow file version");
        ...
        for (WorkflowNode node : wd.getWorkflowNodes()) {
            ...
            if (compId.startsWith("de.rcenvironment.inputprovider/")) {
                ...
                for (EndpointDescription outputEndpoint : node.getOutputDescriptionsManager().getDynamicEndpointDescriptions()) {
                    checkEndpoint(outputEndpoint, "input directory source", "input", DataType.DirectoryReference, WF_PLACEHOLDER_INPUT_DIR,
                        foundInputDirSource, outputReceiver);
                    checkEndpoint(outputEndpoint, "input parameters source", "parameters", DataType.ShortText, WF_PLACEHOLDER_PARAMETERS,
                ...
                if (WF_PLACEHOLDER_OUTPUT_PARENT_DIR.equals(selectedRoot)) {
                    for (EndpointDescription inputEndpoint : node.getInputDescriptionsManager().getDynamicEndpointDescriptions()) {
                        checkEndpoint(inputEndpoint, "output directory receiver", "output", DataType.DirectoryReference, null,
    ...
    private void checkEndpoint(EndpointDescription endpoint, String description, String expectedName, DataType expectedDataType,
        ...
        // minor hack to satisfy the CheckStyle "<= 6 parameters" rule: derive isInputSide value from the fact if a placeholderMarker is set
        final boolean isInputSide = placeholderMarker != null;
