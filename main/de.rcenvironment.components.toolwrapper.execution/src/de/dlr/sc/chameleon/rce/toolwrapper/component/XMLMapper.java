...
//Deprecated, use the new XMLMapper interface (de.rcenvironment.core.utils.common.xml.api.XMLMapper) instead.
@Deprecated
public class XMLMapper {
    ...
    private Document createXPathMappings(final Node sourceNode, final XMLMappingInformation mapInfo) {
        try {
            ...
            InputStream inStream = this.getClass().getResourceAsStream(COMMON.getString("formatter.xslt"));
            if (inStream == null) {
                //Fallback; when running JUnit Tests from Eclipse the above location cannot be resolved.
                inStream = getClass().getResourceAsStream("/XMLFormatter.xslt");
            }
            ...
            InputStream inStreamXPath = this.getClass().getResourceAsStream(COMMON.getString("xpaths.xslt"));
            if (inStreamXPath == null) {
                //Fallback; when running JUnit Tests from Eclipse the above location cannot be resolved.
                inStreamXPath = getClass().getResourceAsStream("/CreateXPaths.xslt");
            }
