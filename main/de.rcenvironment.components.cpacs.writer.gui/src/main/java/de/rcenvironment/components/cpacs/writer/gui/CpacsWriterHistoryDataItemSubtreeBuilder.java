...
public class CpacsWriterHistoryDataItemSubtreeBuilder implements ComponentHistoryDataItemSubtreeBuilder {
    ...
    static {
        String iconPath = "platform:/plugin/de.rcenvironment.components.cpacs.writer.common/resources/cpacsWriter16.png";
        URL url = null;
        try {
            url = new URL(iconPath);
        } catch (MalformedURLException e) {
            LogFactory.getLog(CpacsWriterHistoryDataItemSubtreeBuilder.class).error("Component icon not found: " + iconPath);
        }
        if (url != null) {
